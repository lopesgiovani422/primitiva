...
export default function Services() {
    ...
    const toggleAccordion = (index: number) => {
        ...
        // PostHog: Track service expanded (only when opening)
        if (isOpening && services[index]) {
            posthog.capture('service_expanded', {
                    ...
                    <div className="md:col-span-8 flex flex-col">
                        {services.map((item, idx) => (
                            <div key={idx} className={`border-b border-white/10 reveal-up delay-${(idx + 1) * 100}`}>
                                ...
                                >
                                    <span className="text-[clamp(1.1rem,5.5vw,1.5rem)] md:text-4xl font-bold uppercase tracking-tight">{item.title}</span>
